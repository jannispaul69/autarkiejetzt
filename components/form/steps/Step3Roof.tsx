...
import { Sun, Compass, Navigation, HelpCircle } from "lucide-react";

type RoofOrientation = "south" | "east_west" | "north" | "unknown";

interface Props {
  onComplete: (value: RoofOrientation) => void;
  onBack: () => void;
  defaultValue?: RoofOrientation;
}
...
const OPTIONS: Option[] = [
  {
    value: "south",
    label: "Süd / Südost / Südwest",
    badge: "Optimal für Solar",
    Icon: Sun,
  },
  {
    value: "east_west",
    label: "Ost-West",
    hint: "Sehr gut geeignet",
    Icon: Compass,
  },
  {
    value: "north",
    label: "Nord",
    hint: "Eingeschränkt geeignet",
    Icon: Navigation,
  },
  {
    value: "unknown",
    label: "Weiß ich nicht",
    hint: "Kein Problem – Berater prüft das",
    Icon: HelpCircle,
  },
];
...
function OptionCard({
  ...
  const { Icon, label, hint, badge } = option;
  return (
    <motion.button
      type="button"
    ...
    >
      ...
      <div className="flex flex-col min-w-0">
        ...
        {badge && (
          <span className="mt-1 inline-flex self-start px-1.5 py-0.5 rounded text-xs font-medium bg-brand-accent text-brand-text">
            {badge}
          </span>
        )}
        {hint && !badge && (
...
export default function Step3Roof({ onComplete, onBack, defaultValue }: Props) {
  const [selected, setSelected] = useState<RoofOrientation | null>(defaultValue ?? null);

  return (
    <div className="flex flex-col gap-5">
      <p className="font-medium text-brand-text text-[0.9375rem]">
        Wie ist dein Dach ausgerichtet?
      </p>
